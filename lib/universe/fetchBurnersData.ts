...
function aggregateBurners(commits: BurnCommitment[]): BurnersApiResponse {
...
export async function fetchBurnersData(): Promise<BurnersApiResponse> {
  const commits = await fetchAllBurnCommitments();
  return aggregateBurners(commits);
}
